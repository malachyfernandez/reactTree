...
var require_generated = __commonJS({
  "node_modules/@babel/types/lib/validators/generated/index.js"(exports) {
    ...
    exports.isCallExpression = isCallExpression3;
    ...
    exports.isJSXFragment = isJSXFragment3;
    ...
    exports.isParenthesizedExpression = isParenthesizedExpression3;
    ...
    function isCallExpression3(node, opts) {
      if (!node) return false;
    ...
    function isParenthesizedExpression3(node, opts) {
      if (!node) return false;
    ...
    function isJSXFragment3(node, opts) {
      if (!node) return false;
...
var require_constants = __commonJS({
  "node_modules/@babel/types/lib/constants/index.js"(exports) {
    ...
    exports.BLOCK_SCOPED_SYMBOL = /* @__PURE__ */ Symbol.for("var used to be block scoped");
    exports.NOT_LOCAL_BINDING = /* @__PURE__ */ Symbol.for("should not be considered a local binding");
  }
});
...
var CHILDREN_SLOT = "__CHILDREN__";
...
function isChildrenExpression(n) {
  if (!n) return false;
...
function isWithinOrEqual(parentAbs, childAbs) {
  if (path.resolve(parentAbs) === path.resolve(childAbs)) return true;
  const rel = path.relative(parentAbs, childAbs);
  return !rel.startsWith("..") && !path.isAbsolute(rel);
}
function isIgnored(absFile, rootDirAbs, ignoreDirsRel) {
...
// src/parseFile.ts
var EXT_CANDIDATES = [".tsx", ".ts", ".jsx", ".js"];
...
async function resolveImport(fromFileAbs, spec) {
  const fromDir = path2.dirname(fromFileAbs);
  const base = path2.resolve(fromDir, spec);
  ...
  try {
    ...
    if (st.isDirectory()) {
      for (const ext of EXT_CANDIDATES) {
        const idx = path2.join(base, "index" + ext);
...
function getReturnedJsxFromFunctionBody(body) {
  for (const st of body.body) {
    if (t2.isReturnStatement(st)) {
      const arg = st.argument;
      if (arg && (t2.isJSXElement(arg) || t2.isJSXFragment(arg))) return arg;
    }
  }
  return null;
}
function getJsxFromExpression(expr) {
  if (t2.isJSXElement(expr) || t2.isJSXFragment(expr)) return expr;
  if (t2.isBlockStatement(expr)) return getReturnedJsxFromFunctionBody(expr);
  if (t2.isParenthesizedExpression(expr)) return getJsxFromExpression(expr.expression);
  return null;
}
function findDefaultExportComponentJsx(ast) {
  let defaultExportId = null;
  let jsx = null;
  for (const st of ast.program.body) {
    if (!t2.isExportDefaultDeclaration(st)) continue;
    const decl = st.declaration;
    if (t2.isFunctionDeclaration(decl) && decl.body) {
      return getReturnedJsxFromFunctionBody(decl.body);
    }
    if (t2.isArrowFunctionExpression(decl)) {
      return getJsxFromExpression(decl.body);
    }
    if (t2.isIdentifier(decl)) {
      defaultExportId = decl.name;
    }
    if (t2.isCallExpression(decl)) {
      for (const arg of decl.arguments) {
        if (t2.isArrowFunctionExpression(arg) || t2.isFunctionExpression(arg)) {
          if (t2.isBlockStatement(arg.body)) {
            const found = getReturnedJsxFromFunctionBody(arg.body);
            if (found) return found;
          } else if (t2.isJSXElement(arg.body) || t2.isJSXFragment(arg.body)) {
            return arg.body;
          }
        }
      }
  ...
  traverse(ast, {
    ...
    VariableDeclarator(p) {
      const n = p.node;
      if (!t2.isIdentifier(n.id) || n.id.name !== defaultExportId) return;
      const init = n.init;
      if (t2.isArrowFunctionExpression(init)) {
        const found = getJsxFromExpression(init.body);
        if (found) jsx = found;
        p.stop();
      }
      if (t2.isFunctionExpression(init)) {
...
async function parseReactFile(opts) {
...
async function expandNode(opts, node, importMap, ctx) {
  ...
  try {
    ...
    const expandedChildren = await Promise.all(
      stitched.map(
        (c) => expandNode(opts, c, mergedImportMap, {
          ...ctx,
          depth: ctx.depth + 1,
    ...
    return { name: node.name, children: expandedChildren, expandedFromFile: path3.relative(opts.rootDir, maybeFile) };
  } catch (e) {
    ctx.warnings.push(`Failed to parse ${maybeFile}: ${e.message}`);
    return node;
  }
}
async function buildJsxTree(opts) {
  const warnings = [];
...
function isWrapperFragment(n) {
  return n.name === "Fragment";
}
function renderNode(n, indent, out, opts) {
...
function nodeIdPath(pathIds) {
  return pathIds.join("/");
}
function isToggleable(node) {
  return !!node.expandedFromFile;
}
function collectToggleables(root, pathIds = [], out) {
...
function renderDomLines(root, collapsedIds, pathIds = [], indentLevel = 0, lines = []) {
  const myPath = [...pathIds, root.name];
  const id = nodeIdPath(myPath);
...
function startInteractiveUI(opts) {
  const collapsedIds = opts.collapsedIds ?? /* @__PURE__ */ new Set();
  ...
  const header = blessed.box({
    ...
    height: 3,
    tags: false,
    content: `${pc2.bold(pc2.cyan(opts.title))}
${pc2.dim(opts.metaLine)}
${pc2.dim(
  ...
  screen.append(list);
  const initCollapseDefaults = (root) => {
    const allToggleables = /* @__PURE__ */ new Set();
...
function parseIgnoreList(raw) {
  ...
  const s = String(raw);
  if (!s.trim()) return [];
  return s.split(",").map((x) => normalizeRelDir(x)).filter(Boolean);
}
async function runOnce(opts) {
  const res = await buildJsxTree(opts);
  const header = pc3.bold(pc3.cyan("JSX Tree")) + pc3.dim(`  root=${toPosixPath(path4.relative(process.cwd(), opts.rootDir))}`);
  const meta = pc3.dim(
    `start=${toPosixPath(path4.relative(opts.rootDir, opts.startFile))}  depth<=${opts.maxDepth}  files=${res.filesTouched.size}`
  );
  clearScreen();
  ...
  if (res.warnings.length) {
    process.stdout.write("\n" + pc3.yellow(pc3.bold("Warnings")) + "\n");
    for (const w of res.warnings.slice(0, 25)) process.stdout.write(pc3.yellow(`- ${w}
`));
    if (res.warnings.length > 25) process.stdout.write(pc3.yellow(`- ... (${res.warnings.length - 25} more)
`));
  }
}
function createFocusedWatcher(params) {
  ...
  const watched = new Set(Array.from(params.initialFiles));
  const syncFiles = async (nextFiles) => {
...
async function main() {
  const argv = await yargs(hideBin(process.argv)).scriptName("react-jsx-tree").usage("$0 <startFile> [options]").positional("startFile", { type: "string", describe: "Entry file (tsx/jsx) to start from" }).option("watch", { type: "boolean", default: false, describe: "Live update on file changes" }).option("ignore", {
    type: "string",
    default: "",
    describe: "Comma-separated folder(s) under the start file folder to ignore (ex: ui,components/ui)"
  }).option("depth", { type: "number", default: 25, describe: "Max component expansion depth" }).option("show-expanded-from", { type: "boolean", default: false, describe: "Show which file expanded a component" }).option("interactive", { type: "boolean", default: false, describe: "Interactive tree: arrow keys + Enter to expand/collapse" }).option("mouse", { type: "boolean", default: false, describe: "Enable mouse support (click to toggle) in interactive mode" }).demandCommand(1).help().parse();
  const startArg = String(argv._[0]);
  const startFile = path4.resolve(process.cwd(), startArg);
  await ensureFileExists(startFile);
  const rootDir = path4.dirname(startFile);
  ...
  if (argv.interactive) {
    const collapsedIds = /* @__PURE__ */ new Set(["Fragment", "NoJSXReturnFound"]);
    let ui = null;
    let focusedWatcher2 = null;
    let running2 = false;
    let queued2 = false;
    const rerun2 = async () => {
      if (running2) {
      ...
      try {
        const res = await buildJsxTree(opts);
        const metaLine = `start=${toPosixPath(path4.relative(opts.rootDir, opts.startFile))}  depth<=${opts.maxDepth}  files=${res.filesTouched.size}`;
        ...
        if (!ui) {
          ...
          if (argv.watch) {
            focusedWatcher2 = createFocusedWatcher({
        ...
        if (argv.watch && focusedWatcher2) {
          await focusedWatcher2.syncFiles(res.filesTouched);
        }
      } catch (e) {
        clearScreen();
        process.stdout.write(pc3.red(pc3.bold("Error")) + "\n\n");
        process.stdout.write(pc3.red(e.stack || String(e)) + "\n");
      } finally {
        running2 = false;
        if (queued2) {
          queued2 = false;
    ...
    process.on("SIGINT", async () => {
      await focusedWatcher2?.watcher.close();
      ui?.destroy();
      process.exit(0);
    });
    return;
